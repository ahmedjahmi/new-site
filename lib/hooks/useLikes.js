...
export default function useLikes() {
	...
	const endpoint = () => {
		return `${process.env.NEXT_PUBLIC_LOCALHOST}/api/likes/${articleId}`;
	};

	const fetcher = (endpoint) =>
		axios.get(endpoint).then((res) => res.data.likes.likes);

	const { data: likes, mutate, error: swrError } = useSWR(endpoint, fetcher);
	...
	const onLike = async (e) => {
		...
		try {
			...
			// return res.data;
			await axios
				.put(`${process.env.NEXT_PUBLIC_LOCALHOST}/api/likes/${articleId}`)
				.then((res) => res.data);
			await mutate();
		} catch (error) {
