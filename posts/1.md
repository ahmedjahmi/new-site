...
You're bestfriend lives in Brooklyn (Kings County) on Nostrand Ave., so you think he'd get excited to hear that Jay-Z gave him a shoutout. Naturally, you want to share the song with a him.

## The Problem:

You're listening to the song on Apple Music but your friend is an avid Spotify fanatic. If you send them an Apple Music link, they will surely end your friendship because you should already know their position and there is nothing they can do with that link, other than make note of the song to look up later.

Similarly, that problem exists for you as well when they send you a Spotify link.
...
That would be amazing. Let's make that magic SongLink Converter using the Apple Shortcuts app!

Luckily for us, there is a website called [odesli.co](https://odesli.co/) that allows you to paste a song link from _any_ major platforms into their search bar and generates links for that song on **_every_** major platform. Thankfully, they have an open API that we can use in our shortcut. All we need to do is make a request to that api endpoint that contains our song link as a url query parameter.
...
2. The user receives a song link to another platform, and they want to open it with the platform they use.
